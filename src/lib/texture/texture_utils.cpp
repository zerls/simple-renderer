...
bool TgaUtils::loadFromFile(const std::string& filename, 
...
bool TgaUtils::saveToFile(const std::string& filename, 
...
bool TgaUtils::saveGrayscaleToFile(const std::string& filename, 
...
bool TgaUtils::saveDepthToFile(const std::string& filename,
    ...
    if (pseudocolor) {
        // 使用假彩色
        imageData = convertDepthToPseudocolor(depthData, width, height, minDepth, maxDepth, false);
        return saveToFile(filename, imageData, width, height,3);
    } else {
        // 使用灰度图
...
bool loadTextureFromFile(const std::string& filename, 
    ...
    switch (format) {
        case TextureFileFormat::TGA:
            return TgaUtils::loadFromFile(filename, data, width, height, channels);
        case TextureFileFormat::PPM:
            return PpmUtils::loadFromFile(filename, data, width, height, channels);
...
bool saveTextureToFile(const std::string& filename, 
    ...
    switch (format) {
        case TextureFileFormat::TGA:
            return TgaUtils::saveToFile(filename, pixelData, width, height, channels);
        case TextureFileFormat::PPM:
            if (channels == 1) {
                return PpmUtils::saveGrayscaleToFile(filename, pixelData, width, height);
                ...
                if (channels == 4) {
                    ...
                    return PpmUtils::saveToFile(filename, rgbData, width, height);
                } else {
                    return PpmUtils::saveToFile(filename, pixelData, width, height);
                }
            } else {
        ...
        case TextureFileFormat::AUTO:
            ...
            if (extension == ".tga") {
                ...
                if (channels == 1) {
                    ...
                    return PpmUtils::saveGrayscaleToFile(filename, grayData, width, height);
                }
            } else {
                std::cerr << "不支持的文件格式: " << extension << std::endl;
                return false;
...
bool saveDepthToFile(const std::string& filename,
    ...
    switch (format) {
        case TextureFileFormat::TGA:
            return TgaUtils::saveDepthToFile(filename, depthData, width, height, minDepth, maxDepth, pseudocolor);
        case TextureFileFormat::PPM:
            return PpmUtils::saveDepthToFile(filename, depthData, width, height, minDepth, maxDepth, pseudocolor);
        case TextureFileFormat::AUTO:
            // 根据文件扩展名自动选择
            std::string extension = std::filesystem::path(filename).extension().string();
            std::transform(extension.begin(), extension.end(), extension.begin(), ::tolower);
...
std::vector<uint8_t> convertDepthToPseudocolor(
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            // 转换为假彩色（从蓝到红的热力图）
            float r = std::clamp(normalizedDepth * 2.0f, 0.0f, 1.0f);
            float g = std::clamp(2.0f * std::abs(normalizedDepth - 0.5f), 0.0f, 1.0f);
            float b = std::clamp(2.0f * (1.0f - normalizedDepth), 0.0f, 1.0f);
            
            // 设置RGB值
            rgbData[destIndex] = static_cast<uint8_t>(r * 255.0f);     // R
            rgbData[destIndex + 1] = static_cast<uint8_t>(g * 255.0f); // G
            rgbData[destIndex + 2] = static_cast<uint8_t>(b * 255.0f); // B
        }
    }
...
void TgaUtils::writeHeader(std::ofstream& file, int width, int height, int channels) {
    uint8_t header[18] = {0};
...
void TgaUtils::writeFooter(std::ofstream& file) {
...
bool PpmUtils::loadFromFile(const std::string& filename, 
                           std::vector<uint8_t>& data, 
                           int& width, 
                           int& height,
                           int& channels) {
...
bool PpmUtils::saveToFile(const std::string& filename, 
                          const std::vector<uint8_t>& rgbData, 
                          int width, 
                          int height) {
...
bool PpmUtils::saveGrayscaleToFile(const std::string& filename, 
                                  const std::vector<uint8_t>& grayData, 
                                  int width, 
                                  int height) {
    if (grayData.empty() || width <= 0 || height <= 0 || grayData.size() < width * height) {
...
bool PpmUtils::saveDepthToFile(const std::string& filename,
                               const std::vector<float>& depthData,
                               int width,
                               int height,
                               float minDepth,
    ...
    if (pseudocolor) {
        // 使用假彩色
        imageData = convertDepthToPseudocolor(depthData, width, height, minDepth, maxDepth, false);
        return saveToFile(filename, imageData, width, height);
    } else {
        // 使用灰度图
        imageData = convertDepthToGrayscale(depthData, width, height, minDepth, maxDepth, false);
        return saveGrayscaleToFile(filename, imageData, width, height);
