...
float4 Texture::read(int x, int y, int level) const
{
    ...
    return texture_utils::convertToFloat4(pixelData, format);
}
...
void Texture::write(int x, int y, const float4 &color, int level)
{
    ...
    texture_utils::convertFromFloat4(pixelData, color, format);
}

// 获取直接像素索引
size_t Texture::getPixelIndex(int x, int y, int width) const
{
    return (y * width + x) * texture_utils::getBytesPerPixelFromFormat(format);
}
...
bool Texture::saveToFile(const std::string &filename, int level, TextureFileFormat format) const
{
    ...
    {
        format = texture_utils::getFormatFromFilename(filename);
    }

    const MipmapLevel &mipLevel = mipLevels[level];

    // 使用工具类保存文件
    if (!texture_io::saveTextureToFile(
...
bool Texture::create(int width, int height, TextureFormat format, TextureAccess access, bool generateMipmaps)
{
    ...
    // 分配内存
    int bytesPerPixel = texture_utils::getBytesPerPixelFromFormat(format);
...
bool Texture::loadFromFile(const std::string &filename, TextureFileFormat fileformat)
{
    ...
    {
        fileformat = texture_utils::getFormatFromFilename(filename);
    }
    ...
    // 使用工具类加载文件
    if (!texture_io::loadTextureFromFile(filename, fileData, fileWidth, fileHeight, fileChannels, fileformat))
    {
...
bool Texture::generateMipmaps()
{
    ...
    for (int level = 1; level < maxLevels; ++level)
    {
        ...
        // 生成下一级mipmap
        if (!texture_utils::generateNextMipLevel(currentLevel, nextLevel, format))
        {
...
{
    return texture_utils::getChannelsFromFormat(format);
}
...
float4 Texture::sampleGrad(float2 uv, float2 ddx, float2 ddy, const SamplerState &samplerState) const{
    return TextureSampler::sampleGrad(this, uv,ddx,  ddy, samplerState);
}
float Texture::sampleDepth(float2 uv, const SamplerState &samplerState) const{
    return TextureSampler::sampleDepth(this, uv, samplerState);
}

int Texture::getBytesPerPixel() const
{
    return texture_utils::getBytesPerPixelFromFormat(format);
}
...
{
    // 加载纹理工厂函数
    std::shared_ptr<Texture> loadTexture(const std::string &filename, TextureFileFormat format)
    {
